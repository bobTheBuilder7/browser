...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        plug = QAction(QIcon(os.path.join('icons', 'cil-library-add.png')), "Plugin", self)
        new_tab_action.setStatusTip("Open a new plugin window")
        file_menu.addAction(plug)
        plug.triggered.connect(self.run_script)
        # ADD NEW TAB
    ...
    def run_script(self):
        try:
            ...
            plugins[self.tabs.currentWidget().page()] = fname
        except:
            error = Error()
            error.show()
    ...
    def navigate_to_url(self):  # Does not receive the Url
        ...
        cur = con.cursor()
        url = q.url()
        cur.execute('INSERT INTO history(url) VALUES (?)', (url,))
        con.commit()
        if q.scheme() == "":
